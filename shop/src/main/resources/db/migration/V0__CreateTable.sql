...
create table order_detail (quantity integer not null, location_id uuid not null , order_id uuid not null , product_id uuid not null, primary key (order_id, product_id));
create table placed_order (created_at timestamp(6) not null, customer_id uuid, id uuid not null default uuid_generate_v4(), city varchar(255), country varchar(255), county varchar(255), street_address varchar(255), primary key (id));
...
alter table if exists order_detail add constraint FKgnpelk7hgw6afqum0fw7q878n foreign key (order_id) references placed_order;
